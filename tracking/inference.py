...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for i in range(1, len(values)):
            values[i] += values[i-1]
            if sample < values[i] and sample >= values[i-1]:
                return keys[i-1]
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        B = self.beliefs.copy()
        for W in self.allPositions:
            self.beliefs[W] = \
                self.getObservationProb(observation, gameState.getPacmanPosition(), W, self.getJailPosition())*B[W]
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.allPositions:
            dists[pos] = self.getPositionDistribution(gameState, pos)

        newBelief = DiscreteDistribution()
        for W in self.allPositions:
            dist = dists[W]
            for w in self.allPositions:
                newBelief[w] += self.beliefs[W]*dist[w]
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        for _ in range(self.numParticles):
            self.particles.append(random.choice(positions))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        weights = DiscreteDistribution()
        for particle in self.particles:
            weightList = [self.getObservationProb(observation[i], pacmanPosition, particle[i], self.getJailPosition(i)) for i in range(self.numGhosts)]
            w = 1.0
            for i in weightList:
                w *= i
