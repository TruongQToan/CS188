    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        mostLikelyPositions = [belief.argMax() for belief in livingGhostPositionDistributions]
        successorPositions = [(action, Actions.getSuccessor(pacmanPosition, action)) for action in legal]
        bestAction = None
        minDist = float('inf')
        for action, succ in successorPositions:
            for pos in mostLikelyPositions:
                dist = self.distancer.getDistance(succ, pos)
                if  dist < minDist:
                    bestAction = action
                    minDist = dist
